...
BATCHSIZE = 32
train_dataload = Data.DataLoader(dataset= data, batch_size= BATCHSIZE, shuffle= True)
...
def train(train_dataload,EPOCH):
    ...
    optimizer = torch.optim.Adam(params, lr= 0.001,weight_decay=0.001/EPOCH)
    ...
    for epoch in range(EPOCH):
      ...
      for batch_idx, (data,target) in enumerate(train_dataload):
        ...
        acc = (torch.max(output.cpu(), 1)[1].data.numpy().squeeze() == target.cpu().numpy()).sum()/target.shape[0]
        acc_all += acc
        # if batch_idx % 10 ==0:
            # print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
            #     epoch,batch_idx*len(data),len(train_dataload.dataset),
            #     100. *batch_idx/len(train_dataload), loss.item()))
        print('acc_all :{} '.format(acc))
#创建网络
class Net(nn.Module):
